...
/* adding today */
        ...
        while (hasAugmentingPath(G, s, t))
        {
            ...
            // Walk the path from t back to s
            for (int v = t; v != s; v = edgeTo[v].other(v))
            {
                bottleneck = Math.min(bottleneck, edgeTo[v].residualCapacityTo(v));
            }
            // Know we know the bottleneck for the current augmenting path
            for (int v = t; v != s; v = edgeTo[v].other(v))
            {
    ...
    private boolean hasAugmentingPath(FlowNetwork G, int s, int t) {
        ...
        while (!queue.isEmpty() && !marked[t]) {
            ...
            for (FlowEdge e : G.adj(v)) {
                ...
                if (e.residualCapacityTo(w) > 0) { // mostly checks for exhausted edges
                    if (!marked[w]) { // not needed if graph is acyclic
                        edgeTo[w] = e;
                        marked[w] = true;
                        queue.enqueue(w);
                    }
    ...
    public static void main(String[] args) {
        ...
        int V = 5;
        int s = 0, t = 4, a = 1, b = 2, c = 3;
        FlowNetwork G = new FlowNetwork(V);
        G.addEdge(new FlowEdge(s,a,3.0));
        G.addEdge(new FlowEdge(a,c,1.0));
        G.addEdge(new FlowEdge(a,t,1.0));
        G.addEdge(new FlowEdge(s,b,7.0));
        G.addEdge(new FlowEdge(b,c,9.0));
        G.addEdge(new FlowEdge(c,t,5.0));
